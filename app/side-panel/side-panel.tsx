...
export const Text = ({ onClose }: { onClose?(): void }) => (
  <>
    ...
    <br />
    <Typography.Text tag="p">
      Требования 115-ФЗ и связанных с ним документов Банка России часто
      меняются, предприниматели не всегда успевают за ними следить. Последствия
      нарушений «антиотмывочного» законодательства всегда неприятны: приходится
      остановить бизнес-процессы и доказать банку законность операций.
      Специалисты «Альфа-банка» собрали понятные рекомендации, как сэкономить
      время на объяснения и предотвратить блокировки
    </Typography.Text>
...
export const SidePanel = () => {
  ...
  const [headerViewSelected, setHeaderView] = React.useState<any>(
    HEADER_SELECT_OPTIONS[0].key,
  );
  const [footerViewSelected, setFooterView] = React.useState<any>(
    FOOTER_SELECT_OPTIONS[0].key,
  );
  const [headerSettings, setHeaderSettings] = React.useState<any>(() => ({
    ...HEADER_SETTINGS.reduce(
      (res, item) => ({ ...res, [item.name]: false }),
      {},
    ),
    titleInContent: false,
  }));
  const [footerSettings, setFooterSettings] = React.useState<any>(() =>
  ...
  const showFooter =
    footerSettings.hasPrimaryButton || footerSettings.hasSecondaryButton;
  ...
  return (
    <>
      ...
      <SidePanelMobile open={open} onClose={handleClose} key={getKey()}>
        ...
        {showFooter && (
          <SidePanelMobile.Footer sticky={footerSettings.sticky}>
            <SidePanelMobile.Controls
              layout={
                footerViewSelected === 'vertical' ? 'column' : 'space-between'
              }
              primary={
